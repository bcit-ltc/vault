...
```bash
sops -e --hc-vault-transit https://vault.ltc.bcit.ca:8200/v1/sops/keys/gitops-key BCIT_Rancher_Signing.key > BCIT_Rancher_Signing.key.encrypted
```

1. Confirm decryption works

```bash
sops -d BCIT_Rancher_Signing.key.encrypted
```

1. Remove the secret

```bash
rm BCIT_Rancher_Signing.key
```
